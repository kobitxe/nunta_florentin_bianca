export default {
  nav: {
    detalii: 'Detalles',
    program: 'Programa',
    galerie: 'Galería',
    rsvp: 'Confirmar',
  },
  carta: {
    abrir: 'Abre la invitación',
  },
  hero: {
    anunt: 'Nos casamos',
    si: '&',
    datele: '5 y 8 de agosto de 2027',
    orase: 'Brăila · Mamaia',
    countdown: {
      titlu: 'Para la ceremonia',
      zile: 'días',
      ore: 'horas',
      minute: 'minutos',
      secunde: 'segundos',
    },
    cta: 'Confirmar asistencia',
  },
  detalii: {
    eyebrow: 'Dos momentos, dos ciudades',
    titlu: 'Detalles del evento',
    ceremonie: {
      eticheta: 'Ceremonia religiosa',
      data: 'jueves, 5 de agosto de 2027',
      ora: '11:00 h',
      oraNota: 'la hora exacta se confirmará pronto',
    },
    receptie: {
      eticheta: 'La celebración',
      data: 'domingo, 8 de agosto de 2027',
      ora: '17:00 h',
    },
    ventMapa: 'Abrir en Google Maps',
  },
  program: {
    eyebrow: 'Programa de los días',
    titlu: 'Así lo celebraremos',
    zi1: '5 de agosto — Brăila',
    zi2: '8 de agosto — Mamaia',
    evenimente1: [
      { ora: '10:30', text: 'Llegada de los invitados a la iglesia' },
      { ora: '11:00', text: 'Ceremonia religiosa' },
      { ora: '12:30', text: 'Felicitaciones y fotografías' },
    ],
    evenimente2: [
      { ora: '17:00', text: 'Recepción de invitados en la terraza' },
      { ora: '18:00', text: 'Aperitivo con vistas al mar' },
      { ora: '19:30', text: 'Cena de gala' },
      { ora: '21:00', text: 'Primer baile' },
      { ora: '00:00', text: 'Tarta nupcial' },
    ],
  },
  galerie: {
    eyebrow: 'Nuestra historia',
    titlu: 'Galería de fotos',
    inCurand: 'Las fotografías aparecerán aquí muy pronto.',
  },
  rsvp: {
    eyebrow: 'Te esperamos',
    titlu: 'Confirmar asistencia',
    intro: 'Por favor, dinos antes del 1 de junio de 2027 si nos acompañarás.',
    nume: 'Nombre y apellidos',
    numePlaceholder: 'p. ej. Ana García',
    email: 'Email (opcional)',
    telefon: 'Teléfono (opcional)',
    asistencia: '¿Asistirás?',
    da: 'Sí, allí estaré',
    nu: 'Lamentablemente, no puedo',
    acompanantes: 'Número de acompañantes',
    restricciones: 'Restricciones alimentarias (opcional)',
    restriccionesPlaceholder: 'p. ej. vegetariano, alergia a los frutos secos…',
    mensaje: 'Un mensaje para los novios (opcional)',
    mensajePlaceholder: 'Escríbenos unas palabras…',
    enviar: 'Enviar confirmación',
    enviando: 'Enviando…',
    gracias: '¡Gracias! Tu confirmación ha quedado registrada.',
    graciasNo: 'Gracias por avisarnos. ¡Te echaremos de menos!',
    error: 'No hemos podido enviar la confirmación. Inténtalo de nuevo.',
    faltaNombre: 'Por favor, dinos tu nombre.',
    sinConfig: 'El formulario estará activo muy pronto.',
  },
  stats: {
    eyebrow: 'En tiempo real',
    titlu: 'Quién viene a la boda',
    confirmados: 'invitados confirmados',
    personas: 'personas en total',
    respuestas: 'respuestas recibidas',
    noAsisten: 'no pueden asistir',
    sinConfig: 'Las estadísticas aparecerán aquí tras la configuración.',
    vacio: 'Aún no hay confirmaciones. ¡Sé el primero!',
  },
  footer: {
    frase: 'Con cariño, os esperamos para celebrarlo juntos.',
    firma: 'Flo & Bianca · agosto 2027',
  },
}
